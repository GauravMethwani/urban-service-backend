...
const validationMiddleware = (useJoiError = false) => {
  ...
  return (req, res, next) => {
    ...
    if (_supportedMethods.includes(method) && _.has(validators, route)) {
      ...
      if (_schema) {
        const { error, value } = _schema.validate(_.isEmpty(req.body) ? req.query : req.body, _validationOptions);
        
        if (error) {
          console.log(`[Validation Error on ${route}]:`, error.details[0].message);
          const JoiError = {
            success: false,
